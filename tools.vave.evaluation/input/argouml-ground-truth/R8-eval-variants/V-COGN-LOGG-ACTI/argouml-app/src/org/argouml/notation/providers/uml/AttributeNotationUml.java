...
public class AttributeNotationUml extends AttributeNotation {
	private static final Logger LOG = Logger.getLogger(AttributeNotationUml.class);
	...
	protected void parseAttribute(String text,Object attribute)throws ParseException {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("ParseAttribute [name: " + name + " visibility: " + visibility + " type: " + type + " value: " + value + " stereo: " + stereotype + " mult: " + multiplicity);
			if (properties != null) {
				for (int i = 0;i + 1 < properties.size();i += 2) {
					LOG.debug("\tProperty [name: " + properties.get(i) + " = " + properties.get(i + 1) + "]");
				}
			}
		}
