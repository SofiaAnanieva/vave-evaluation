...
public class CrInvalidSynch extends CrUML {
	...
	public boolean predicate2(Object dm,Designer dsgr) {
		...
		while (outgoing.hasNext()) {
			...
			if (destinationRegion == null)destinationRegion = aux;else if (!aux.equals(destinationRegion))return PROBLEM_FOUND;
		}
		...
		while (incoming.hasNext()) {
			...
			if (sourceRegion == null)sourceRegion = aux;else if (!aux.equals(sourceRegion))return PROBLEM_FOUND;
		}
		if (destinationRegion != null&&!Model.getFacade().isAConcurrentRegion(destinationRegion))return PROBLEM_FOUND;
		if (sourceRegion != null&&!Model.getFacade().isAConcurrentRegion(sourceRegion))return PROBLEM_FOUND;
		return NO_PROBLEM;
	}
