...
public class ClassdiagramLayouter implements Layouter {
	...
	public NodeRow doSplit(int maxWidth,int gap) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Row split. This row width: " + getWidth(gap) + " next row(s) width: " + newRow.getWidth(gap));
		}
		return newRow;
	}
	...
	private void placeNode(ClassdiagramNode node) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("placeNode - Row: " + node.getRank() + " Col: " + node.getColumn() + " Weight: " + node.getWeight() + " Position: (" + xPosNew + "," + yPos + ") xPos: " + xPos + " hint: " + node.getPlacementHint());
		}
		if (downlinks.size() == 1) {
