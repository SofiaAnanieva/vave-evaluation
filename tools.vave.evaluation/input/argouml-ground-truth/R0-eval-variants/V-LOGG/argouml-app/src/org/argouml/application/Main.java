...
public class Main {
	...
	if (System.getProperty("log4j.configuration") == null) {
		...
		try {
			stream = Thread.currentThread().getContextClassLoader().getResourceAsStream(DEFAULT_LOGGING_CONFIGURATION);
			if (stream != null) {
				props.load(stream);
			}
		...
		if (stream == null) {
			...
			Logger.getRootLogger().getLoggerRepository().setThreshold(Level.ERROR);
			Logger.getRootLogger().error("Failed to find valid log4j properties" + "in log4j.configuration" + "using default logging configuration");
		}
	}
