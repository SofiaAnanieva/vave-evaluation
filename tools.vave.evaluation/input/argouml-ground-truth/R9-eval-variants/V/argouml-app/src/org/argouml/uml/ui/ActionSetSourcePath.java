...
public class ActionSetSourcePath extends UndoableAction {
	...
	@Override public void actionPerformed(ActionEvent e) {
		...
		if (f != null) {
			...
			if (Model.getFacade().isAModelElement(obj)) {
				...
				if (tv == null) {
					Model.getExtensionMechanismsHelper().addTaggedValue(obj,Model.getExtensionMechanismsFactory().buildTaggedValue(ImportInterface.SOURCE_PATH_TAG,f.getPath()));
				}else {
	...
	protected File getNewDirectory() {
		...
		int retval = chooser.showDialog(ArgoFrame.getInstance(),Translator.localize("dialog.button.ok"));
