	...
	private void parseTrigger(Object trans,String trigger)throws ParseException {
		...
		if (trigger.length() > 0) {
			if (evt == null) {
				...
				if (callEvent) {
					String triggerName = trigger.indexOf("(") > 0?trigger.substring(0,trigger.indexOf("(")).trim():trigger;
	...
	private void parseGuard(Object trans,String guard) {
		...
		if (guard.length() > 0) {
			if (g == null) {
				...
				if (g != null) {
					Model.getStateMachinesHelper().setExpression(g,Model.getDataTypesFactory().createBooleanExpression("",guard));
				...
				Model.getStateMachinesHelper().setExpression(g,Model.getDataTypesFactory().createBooleanExpression(language,guard));
			}
		}else {
			if (g == null) {
			}else {
